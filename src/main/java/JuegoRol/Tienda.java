...
// Tienda del Viajero. Aparece entre oleadas y le vende cosas al jugador.
public class Tienda {

    // Genera el stock que hay disponible. A mas nivel, mejores cosas.
    public static ArrayList<Objeto> generarStock(int nivelJugador){
        ...
        // ─── Bombas, pa cuando un bicho te jode mucho ───
        ...
        // A partir del nivel 3 desbloqueo cosas mejores
        if(nivelJugador >= 3){
    ...
    // Abre la tienda y deja al jugador comprar
    public static void abrir(Jugador j, Scanner sc){
        Random rd = new Random();
        ArrayList<Objeto> stock = generarStock(j.getNivel());

        // No enseño todo el stock, solo unos cuantos al azar pa que cambie cada vez
        ...
        while(!salir){
            ...
            try {
                ...
                // Intento gastar el oro, si va ok meto el objeto en el inventario
                if(j.getInventario().gastarOro(o.getPrecio())){
                    if(j.getInventario().agregarObjeto(o)){
                        ...
                        // si el inventario esta lleno, devuelvo el oro (sino se queda sin nada el pobre)
